...
这个项目目前是一个**对比实验室**——用 LangChain 文档作为公共语料，得出一份可复现的范式选型报告。但它的核心架构有很强的可复用性：三个范式实现都解耦于具体语料，`fetch_docs.py` 拉哪个仓库的文档都行，`questions.json` 换成任何企业的真实问题集都能跑，judge（Gemini Pro + `read_doc_file` 工具）针对新语料同样有效。沿着这条路再做一些工程化改造，本项目能直接服务以下几类角色和场景。

### 角色一：Forward Deployed Engineer（FDE）/ Solutions Engineer

2026 年是 FDE 这个岗位的爆发期——Palantir、OpenAI、Anthropic、Glean、Decagon 等公司都在大规模招聘，专门帮企业客户把 AI 部署到他们的真实数据上。FDE 面对的高频问题是：

> "客户问我们：他们的文档库该用 RAG、Agentic 还是 GraphRAG？我得在 1-2 周内给出技术架构建议。"

传统答案是做一个 4-6 周的 POC，但客户经常没耐心等。本项目可以把这个分析周期压到 **2-3 天**：

1. **用 mock data 模拟客户文档构成**：拿 5-10% 客户文档样本（脱敏过）或者基于客户产品文档结构合成 mock data，灌进 `backend/raw_docs/`
2. **跟客户对齐 15-20 道有代表性的问题**，按"单跳 / 多跳 / 跨文档"分类写进 `questions.json`
3. **跑全套评测**：`run_one_paradigm.py` 让三个范式都答一遍 → `judge.py` 用 Gemini Pro 打分
4. **输出客户专属决策矩阵**：渲染出 `/matrix` 页 + 每题的 judge 推理（`scored_*.json`）

交付给客户的不再是"看场景吧"这种含糊建议，而是**基于他们自己文档构成 + 自己问题集**的实证报告。FDE 用这套结果直接驱动后续架构决策、报价、SOW（Statement of Work）。

### 角色二：企业内部 AI 架构师

中大型企业（金融、医疗、法律、制造业、SaaS 公司）的内部 AI 团队在搭建知识库 bot、客服 bot、合规问答系统、内部 IT helpdesk 时，同样面对范式选型的痛点。本项目作为**预选型工具**有两种用法：

- **真实数据预演**：把企业自己的部分文档（敏感数据脱敏后）灌进来，看哪个范式在自己语料上表现最好。例如：合规文档强关系链（GraphRAG 大概率赢）vs 客服 FAQ 短查询（RAG 大概率赢）vs 跨部门技术文档综合查询（Agentic 大概率赢）——本项目的 decision matrix 提供了概念框架，企业自己跑一遍能得到自己语料的具体数字
- **Vendor 中立评估**：很多企业在 vendor 之间犹豫（A 家主推 RAG，B 家主推 GraphRAG）。本项目能给出一个**与 vendor 无关的中立基线**，让企业用数据决策而不是被 sales pitch 牵着走

### 角色三：AI 咨询公司 / 解决方案集成商

咨询公司面对的痛点跟 FDE 类似——客户问"该用哪个 paradigm"时，传统答案太抽象，客户不买账。本项目提供一个**可复现的对比模板**：

- 客户提供文档样本 → 在隔离环境跑一遍 → 输出客户专属的报告（含场景化推荐）
- 整个流程 Docker 化后可以做成 SaaS 工具或者咨询包，大幅缩短前期分析周期，让咨询公司的交付物更"硬"（有数据支撑而不是 PPT slides）

### 走向生产的路径（未来工程化方向）

要把这个 lab 升级成真正服务客户的产品，下一步需要：

1. **多租户 + 鉴权**：每个企业有独立 corpus、向量库、知识图谱、API key、billing 隔离
2. **UI 上传 corpus 和问题集**：现在 corpus 是 CLI 拉的、问题集是 JSON 文件，升级成前端可视化的上传 + 编辑界面，让非工程角色（PM、咨询顾问）也能用
3. **评测 dashboard + executive summary 导出**：实时进度可视化 + 一键导出 PDF/HTML 报告给客户决策层
4. **成本投影**：根据测试数据估算"如果上 N 个用户、每天 Q 次查询，每个 paradigm 的月度成本是多少美金 + 多少 GCP/AWS infra"
5. **paradigm 自动调优**：现在 chunk_size、top-k、retrieval weight 都是写死的，加自动调参 + ablation study，让客户得到"你的语料最优配置"而不只是"哪个范式更好"
6. **多 judge 交叉验证**：用 Claude Opus、GPT-5 等不同 judge 模型重跑，输出"分数稳定性"指标，提高客户对结论的信任
7. **隐私合规**：on-prem 部署、客户数据零 retention、SOC2 / ISO 27001 认证——这是进金融、医疗等高敏感行业的硬门槛
...
> 整个项目最核心的产出物——15 道题 × 3 个范式 × 0-3 分，颜色块编码，配场景化推荐。
