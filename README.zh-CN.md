...
> **📸 注**：截图需要你本地启服务后自己截。把图放到 `docs/screenshots/` 目录、按下列文件名命名，下面的链接就会自动渲染。具体需要哪些截图见 [`docs/screenshots/README.md`](./docs/screenshots/README.md)。
...
- Agentic Search 架构借鉴 [`langchain-ai/chat-langchain`](https://github.com/langchain-ai/chat-langchain)（完整 agent loop 重新实现；chat-langchain 当前 master 使用私有 Mintlify API，需要拿不到的凭证——详见项目历史 `PLAN.md` §4.2 的发现）
